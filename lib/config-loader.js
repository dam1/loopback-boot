...
ConfigLoader.loadDataSources = function(rootDir, env) {
 var enviroment = 'datasources';
 if (process.env.VCAP_APPLICATION != null) {
  var envi = JSON.parse(process.env.VCAP_APPLICATION);
  envi = envi.space_name;
  console.log(envi);
  if(envi === 'STAGING'){
   enviroment = 'datasourcesstaging';
  }
  else if( envi === 'PROD'){
   enviroment = 'datasourcesprod';
  }
 }

 return loadNamed(rootDir, env, enviroment, mergeDataSourceConfig);
